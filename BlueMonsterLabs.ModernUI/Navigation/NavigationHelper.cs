...
using BlueMonsterLabs.ModernUI.Windows.Media;
using System;
using System.Linq;
using System.Windows;

namespace BlueMonsterLabs.ModernUI.Windows.Navigation
{
    /// <summary>
    ...
    public static class NavigationHelper
    {
        ...
        public static ModernFrame FindFrame(string name, FrameworkElement context)
        {
            ...
            // collect all ancestor frames (include templated parent descendants so frames inside control templates are found)
            var framesEnumerable = context.AncestorsAndSelf().OfType<ModernFrame>();

            var templatedParent = context.TemplatedParent as DependencyObject;
            if (templatedParent != null)
            {
                // prefer the template implementation root when present
                var root = VisualTreeHelperEx.GetImplementationRoot(templatedParent) as DependencyObject ?? templatedParent;

                // collect ModernFrame instances from the visual descendants of the root
                var stack = new System.Collections.Generic.Stack<DependencyObject>();
                stack.Push(root);
                var descendantFrames = new System.Collections.Generic.List<ModernFrame>();
                while (stack.Count > 0)
                {
                    var d = stack.Pop();
                    var mf = d as ModernFrame;
                    if (mf != null)
                    {
                        descendantFrames.Add(mf);
                    }

                    int cc = System.Windows.Media.VisualTreeHelper.GetChildrenCount(d);
                    for (int i = 0; i < cc; i++)
                    {
                ...
                framesEnumerable = framesEnumerable.Concat(descendantFrames);
            }

            var frames = framesEnumerable.Distinct().ToArray();
